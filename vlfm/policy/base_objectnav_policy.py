...
from ...perception.glee.image_percevior import GLEEPerception
from ..mapping.object_point_cloud_map import ObjectPointCloudMap
from ..mapping.obstacle_map import ObstacleMap
from ..obs_transformers.utils import image_resize
from ..policy.utils.pointnav_policy import WrappedPointNavResNetPolicy
from ..utils.geometry_utils import get_fov, rho_theta
from ..vlm.blip2 import BLIP2Client
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._object_detector = GLEEPerception()
        ...
        self._observations_cache_copy = None
    ...
    def act(
        ...
        # print(f"\n[Decision]")
        # print(f"Target object: {self._target_object}")
        ...
        self._observations_cache_copy = self._observations_cache
    ...
    def _get_object_goal_name(self) -> str:
    ...
    def _get_object_detections(self, img: np.ndarray) -> ObjectDetections:
        ...
        detections, segmentations = self._object_detector.predict(
            image=img, target=target_classes
        )
    ...
    def _pointnav(self, goal: np.ndarray, stop: bool = False) -> Tensor:
        ...
        print(f"_pointnav_stop_radius: {self._pointnav_stop_radius}")
        print(
            f"depth for pointnav: shape = {obs_pointnav['depth'].shape} value = {obs_pointnav['depth']}"
        )
    ...
    def _update_object_map(
        ...
        detections, segmentations = self._get_object_detections(rgb)
...
cs.store(group="policy", name="vlfmconfig_base", node=VLFMConfig())
