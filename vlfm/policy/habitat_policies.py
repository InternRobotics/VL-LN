...
from ..utils.ovon import ovon_objects
from ..vlm.grounding_dino import ObjectDetections
from .base_objectnav_policy import BaseObjectNavPolicy, VLFMConfig
from .itm_policy import ITMPolicy, ITMPolicyV2, ITMPolicyV3

OVON_ID_TO_NAME = [k for k in ovon_objects["sum"].keys()]
...
class HabitatMixin:
    ...
    def from_config(
        ...
        elif "ovon" in config.habitat.dataset.data_path:
    ...
    def act(
        ...
        if self._dataset_type == "hm3d":
            ...
            obs_dict[ObjectGoalSensor.cls_uuid] = OVON_ID_TO_NAME[object_id]
        else:
    ...
    def _cache_observations(
        ...
        semantic = observations["semantic"][0].cpu().numpy()
...
cs.store(group="habitat_baselines/rl/policy", name="vlfmpolicy", node=VLFMPolicyConfig)
