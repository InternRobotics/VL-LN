...
try:
    ...
    from .non_habitat_policy.nh_pointnav_policy import PointNavResNetPolicy
...
def load_pointnav_policy(file_path: str) -> PointNavResNetTensorOutputPolicy:
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5":
            ...
            from .non_habitat_policy.nh_pointnav_policy import PointNavResNetNet
        ...
        if "net.prev_action_embedding_cont.bias" not in ckpt_dict.keys():
            ckpt_dict["net.prev_action_embedding_cont.bias"] = ckpt_dict[
                "net.prev_action_embedding.bias"
            ]
        if "net.prev_action_embedding_cont.weights" not in ckpt_dict.keys():
            ckpt_dict["net.prev_action_embedding_cont.weight"] = ckpt_dict[
        ...
        print(
            f"The following unused keys were not loaded when loading the pointnav policy: {unused_keys}"
        )
        return pointnav_policy
