...
def open3d_dbscan_filtering(
    ...
    """
    Cluster a point cloud using DBSCAN and return the points in the largest non-noise cluster.

    Args:
        points (np.ndarray): An (N, 3) point cloud. Each row is a 3D point (x, y, z).
        eps (float): DBSCAN neighborhood radius.
        min_points (int): Minimum number of neighbors to define a core point in DBSCAN.

    Returns:
        np.ndarray: Points in the largest non-noise cluster. Returns an empty array if all points are noise.
    """
