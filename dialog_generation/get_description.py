...
def get_points_room(points, region_dict, object_dict, poly_type):
    ...
    point_rooms = [
        [
            room.split("/")[0] + "/" + room.split("/")[1].split("_")[0]
            for room in point_room
        ]
    ...
    rooms_object_height = {
        room: [min(heights), max(heights)]
        for room, heights in rooms_object_height.items()
    }
    new_point_rooms = []
    for idx, point_room in enumerate(point_rooms):
        ...
        new_point_room = [
            r
            for r in point_room
            if rooms_object_height[r][0] - 1
            < points[idx][1]
            < rooms_object_height[r][1]
        ]
...
def get_start_description(angle2first_point, height_diff, room=None):
    ...
    if height_diff > 0.1:
        des += (
            str(des.count("\n") + 1)
            + ". "
            + np.random.choice(GO_STAIRS).format(direction="up")
            + ", "
        )
        ...
        des += (
            str(des.count("\n") + 1)
            + ". "
            + np.random.choice(GO_STAIRS).format(direction="down")
            + ", "
        )
    else:
        des += (
            str(des.count("\n") + 1)
            + ". "
            + np.random.choice(FORWARD)
            + " along the direction "
        )
        if abs(angle2first_point) >= 120:
            des += (
                "after you " + np.random.choice(TURN_BACK) + " from your current view, "
            )
        elif angle2first_point > 20:
            des += (
                "after you "
                + np.random.choice(TURN_ANGLE).format(
                    turn="left", angle=int(round(angle2first_point, -1))
                )
            ...
            des += (
                "after you "
                + np.random.choice(TURN_ANGLE).format(
                    turn="right", angle=int(round(abs(angle2first_point), -1))
                )
                + " from your current view, "
            )
        else:
...
def find_shortest_path(env, target_position):
    ...
    Args:
        env (ObjectNavEnv): A Habitat environment instance.
        target_position (Union[Sequence[float], np.ndarray]): Target position coordinates as a NumPy array [x, y, z].

    Returns:
        List[Sequence[float]]: A list of waypoints along the path.
        bool: Whether a valid path was found.
    """
    ...
    sorted_objects = dict(
        sorted(
            ...
            key=lambda x: np.linalg.norm(
                np.array([x[1]["position"][i] for i in [0, 2]])
                - np.array([point_info["position"][i] for i in [0, 2]])
            ),
        )
    )
...
def get_path_description_without_additional_info(
    ...
    if len(path) == 0:
        return ""
    path_info = {
        idx: {"position": i, "calc_trun": False, "turn": []}
    ...
    if height_list is None:
        ...
        assert len(height_list) == len(
            path
        ), "height_list and path have different length"
        for i in range(len(height_list) - 1):
            if height_list[i + 1] - height_list[i] > 0.1:
                path_info[i]["turn"].append("up")
            elif height_list[i + 1] - height_list[i] < -0.1:
                path_info[i]["turn"].append("down")
    calc_turn_indices, _ = sample_points(
        [pi["position"] for pi in path_info.values()], [""] * len(path_info), 1.0
    )
    ...
    move_z_point_to_sky = np.array(
        [path_info[i]["position"] for i in calc_turn_indices]
    ...
    special_point = [
        i for i in path_info.keys() if len(path_info[i]["turn"]) > 0 and i != 0
    ]
    path_description = ""
    # get initial conjunction
    angle2first_point = compute_yaw_rotation(
        orientation,
        path_info[calc_turn_indices[0]]["position"],
        path_info[calc_turn_indices[1]]["position"],
    )
    height_diff = (
        path_info[calc_turn_indices[1]]["position"][1]
        - path_info[calc_turn_indices[0]]["position"][1]
        if height_list is None
        ...
        if len(path_info[i]["turn"]) > 0:
            for turn in path_info[i]["turn"]:
                ...
                if turn > 0:
                    length = round(
                        np.linalg.norm(
                            np.array(path_info[i]["position"])
                            - np.array(path_info[last_special_point]["position"])
                        )
                    )
                    path_description += (
...
def get_path_description(
    ...
    Args:
        ...
        region_dict (Dict[str, Dict[str, Any]]): Region/room metadata used to assign waypoints to rooms and detect 
            room transitions.
        height_list (Optional[list]): Optional per-step height values of length T.
    ...
    if len(path) == 0:
        return ""
    path_info = get_passed_objects_and_regions(
        path, object_dict, region_dict, height_list
    )
    special_point = [
        i
        for i in path_info.keys()
        if (path_info[i]["new_room"] or len(path_info[i]["turn"]) > 0) and i != 0
    ]
    path_description = ""
    # get initial conjunction
    angle2first_point = compute_yaw_rotation(
        orientation, path_info[0]["position"], path_info[1]["position"]
    )
    height_diff = (
    ...
    for i in special_point:
        if (
            path_info[i]["new_room"]
            and object_dict[path_info[i]["object"]]["room"] != "stair region"
        ):
            path_description += (
                np.random.choice(CONJUNCTION)
                + " "
                + np.random.choice(GO_INTO_ROOM).format(
                    room=get_room_name(object_dict[path_info[i]["object"]]["room"])
                )
                + ", "
            )
        if len(path_info[i]["turn"]) > 0:
    ...
    if return_finish:
        path_description += (
            np.random.choice(CONJUNCTION) + " " + np.random.choice(FINISH_DESCRIPTION)
        )
        return path_description
    else:
        return path_description, path_info
...
def get_nearest_object(path, region_dict, object_dict):
    ...
    for k, v in object_dict.items():
        if v["scope"] + "/" + v["room"] in rooms and v["category"] not in [
            ...
            rooms_object_positions[v["scope"] + "/" + v["room"]][k] = np.array(
    ...
    nearest_indices = [
        np.linalg.norm(
            np.array([i[0], i[2]])
            - np.array(list(rooms_object_positions[point_rooms[idx]].values())),
            axis=1,
        ).argmin()
        for idx, i in enumerate(path)
    ]
    ...
    nearest_objects = [
        list(rooms_object_positions[point_rooms[idx]].keys())[nearest_indices[idx]]
...
def sample_points(points, rooms, min_dist=1.0):
    ...
    Args:
        points (List[Tuple[float, float, float]] | np.ndarray): A list of coordinates in the form [(x, y, z), 
            (x, y, z), ...].
...
def find_sharp_turns(path_points, threshold=30):
    ...
    Args:
        path_points (List[Tuple[float, float, float]] | np.ndarray): A list of path points in the form [(x, y, z), 
            (x, y, z), ...].
